...
passport.connect = function ( request, query, profile, next ) {
	...
	if ( !Object.keys ( user ).length ) {
		...
		if ( profile.hasOwnProperty ( 'name' ) ) {
			if ( profile.name.hasOwnProperty ( 'givenName' ) ) {
		...
		if ( provider == "facebook" ) {
			user.picture = "http://graph.facebook.com/" + profile.id + "/picture?type=large";
...
passport.callback = function ( request, response, next ) {
	...
	if ( provider === 'local' && action !== undefined ) {
		...
		else if ( action === 'connect' && request.user ) {
...
passport.sendResetPasswordEmail = function (email, req, cb) {
  sails.models['user'].findOne({email: email})
    .exec(function(err, user) {
      ...
      if (!user) {
        return cb(new Error('Email does not exist'), null);
      }
      sails.models['passport'].findOne({user: user.id})
        .exec(function(err, passport) {
          var resetCode = crypto.randomBytes(20).toString('hex');
          if (err) {
            return cb(err, null);
          }
          if (!passport) {
            return cb(new Error('Passport does not exist'), null);
          }
          passport.resetCode = resetCode;
          delete passport.password;
          passport.save(function(err, model) {
            if (err) {
              return cb(err, null);
            }
            sails.services.email.send({
              recipient: email,
              subject: 'Reset you password',
              templateName: 'resetPassword',
              renderOptions: {
                resetUrl : req.headers.origin + '/password/reset?resetCode=' + resetCode
              }
            }, function(err, resp) {
              if (err) {
                return cb(err, null);
              }
              return cb(null, resetCode);
            })
          })
